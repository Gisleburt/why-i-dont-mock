...
pub fn DddIsAwesome(cx: Scope) -> Element {
    cx.render(rsx!(
        ...
        Notes {
            ...
            p {"DDD obviously encompasses many things but I want to talk about one specific thing"}
        }
    ))
}
...
pub fn HexagonalArchitecture(cx: Scope) -> Element {
    cx.render(rsx!(
        ...
        Notes {
            ...
            "}}
            p {"Instead, think about it as a loose description of how we talk to external systems"}
            p { indoc! {"
                We have two separate systems that we want to talk to each other. Ideally we don't
                want to tightly couple these systems, we want to be able to change the external
                system without a ton of work. For example, what happens if your business, formally
                tied to a large Mongo dataset, decides to break it up into many Postgres databases?
            "}}
        }
    ))
}
...
pub fn PortsAndAdaptors(cx: Scope) -> Element {
    cx.render(rsx!(
        ...
        Notes {
            ...
            p { indoc! {"
                The port is a pure abstraction, without any logic, of how our application would like
                to communicate its needs. This can usually be represented by an Interface.
            "}}
            p { indoc! {"
