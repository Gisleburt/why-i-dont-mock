...
pub fn UnitTestExample(cx: Scope) -> Element {
    cx.render(rsx!(
        ...
        Notes {
            ...
            p { indoc!{ "
                This test simply calls the function with some data and checks we get back what we
                expect to get back. A little over simplified but hopefully gets the idea across 
            " } }
        }
    ))
}
